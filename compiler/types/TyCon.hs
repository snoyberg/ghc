...
module TyCon(
        ...
        AlgTyConParent(..), isNoParent,
        FamTyConFlav(..), Role(..), Promoted(..),
...
data TyCon
  ...
  -- All these constructors are lifted and boxed.
  ...
  | AlgTyCon {
        ...
        algTcParent :: AlgTyConParent, -- ^ Gives the class or family declaration
  ...
  | TupleTyCon {
        ...
        tupRepName     :: Maybe TyConRepName,  -- ^ Nothing for unboxed tuples
        tcPromoted     :: Promoted TyCon       -- ^ Unboxed tuples aren't promoted
    }
  ...
  | PromotedDataCon {          -- See Note [Promoted data constructors]
        tyConUnique :: Unique, -- ^ Same Name and Unique as the
        tyConName   :: Name,   -- ^  underlying data constructor
        tyConArity  :: Arity,
...
instance Outputable AlgTyConParent where
...
-- name, if any
okParent :: Name -> AlgTyConParent -> Bool
...
isNoParent :: AlgTyConParent -> Bool
...
tyConRepName_maybe (PrimTyCon  { primRepName = mb_rep_nm })
  = mb_rep_nm
tyConRepName_maybe (TupleTyCon { tupRepName = mb_rep_nm })
  = mb_rep_nm
...
mkTupleTyCon :: Name
             ...
             -> Promoted TyCon  -- ^ Promoted version
             -> TyCon
mkTupleTyCon name kind arity tyvars con sort rep_nm prom_tc
  = TupleTyCon {
        tyConUnique = nameUnique name,
        tyConName = name,
        tyConKind = kind,
...
isDistinctTyCon (FamilyTyCon { famTcFlav = DataFamilyTyCon {} }) = True
...
isDecomposableTyCon (FamilyTyCon  { famTcFlav = flav }) = isDataFamFlav flav
...
isImplicitTyCon (FamilyTyCon { famTcParent = Just {} }) = True
